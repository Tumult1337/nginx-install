...
// ParseUpstream normalizes a proxy target to "host:port", bracketing bare
// IPv6 literals. Defaults port to 80 if omitted. Accepts:
//   - 1.2.3.4         -> 1.2.3.4:80
//   - 1.2.3.4:8080    -> 1.2.3.4:8080
//   - ::1             -> [::1]:80
//   - [::1]:8080      -> [::1]:8080
//   - host.example.com:8080 -> host.example.com:8080
//   - host.example.com -> host.example.com:80
func ParseUpstream(arg string) (string, error) {
	if arg == "" {
	...
	if strings.Contains(arg, ":") {
		...
		if ip := net.ParseIP(arg); ip != nil {
			if ip.To4() == nil {
				return fmt.Sprintf("[%s]:80", arg), nil
			}
			return fmt.Sprintf("%s:80", arg), nil
		}
		return "", fmt.Errorf("invalid upstream %q", arg)
	}
	// No colon: bare IPv4 or hostname, default :80.
	return fmt.Sprintf("%s:80", arg), nil
}
...
// paths are returned absolute and verified to exist as a directory.
//
// Disambiguation rules:
//   - starts with "/" -> static. Must exist as a directory; error otherwise.
//   - otherwise       -> proxy. ParseUpstream validates.
func ParseTarget(arg string) (TargetKind, string, error) {
	if arg == "" {
	...
	up, err := ParseUpstream(arg)
	if err != nil {
		return TargetUnknown, "", err
	}
	return TargetProxy, up, nil
}
